...
def main(argv=None):
    ...
    # Ask for server name
    serverName = 'graphc-portal1.anu.edu.au'
    serverPort = 6443

    print
    r"Enter the service name in the format <folder>/<name>.<type>."
    ...
    try:
        ...
        print
        "Numerical value not entered for minimum, maximum, or both."
        return

    # Check to make sure that the minimum is not greater than the maximum
    if minInstancesNum > maxInstancesNum:
        print
        "Maximum number of instances must be greater or equal to minimum number."
        return
    ...
    if (response.status != 200):
        httpConn.close()
        print
        "Could not read service information."
        return
    else:
        data = response.read()
        ...
        if not assertJsonSuccess(data):
            print
            "Error when reading service information. " + str(data)
        else:
            print
            "Service information read successfully. Now changing properties..."
        ...
        if (editResponse.status != 200):
            ...
            if not assertJsonSuccess(editData):
                print
                "Error returned while editing service" + str(editData)
            else:
                print
