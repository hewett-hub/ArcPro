...
def update_row():
    ...
    pt, name = get_clinic_values()  # ensure clinic name is valid and get point in case of need for insert.

    result_value = None
    clinic_name = name.upper()
...
def update_totals(clinic, tests_val, confirmed_val):
    fields = ['TotalTests', 'TotalConfirmed']
    where_clause = "Clinic = '{}'".format(clinic)
...
entry_date = arcpy.GetParameter(4)
tests = arcpy.GetParameter(5)
confirmed = arcpy.GetParameter(6)
