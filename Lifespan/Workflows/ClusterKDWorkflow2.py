...
lits_gdb = r'C:\tmp\LITS\LITS_20200729_08_17.gdb'
...
cluster_analysis_gdb = r'C:\tmp\LITS\ClusterAnalyses_08_17.gdb'
...
kd_working_gdb = r'C:\tmp\LITS\KD_working.gdb'
...
kd_analysis_gdb = r'C:\tmp\LITS\KD_Analyses_08_17.gdb'
...
source_locations = {'Incidents': r"C:\tmp\LITS\LITS_Extracts.gdb\Incidents_xy",
                    'Residence': r"C:\tmp\LITS\LITS_Extracts.gdb\Residence_xy"}
...
scratch_workspace = r'C:\tmp\scratch.gdb'
...
class KDHelper(object):
            ...
            for theme_id, theme_result in source_result.items():
                for kd_id, kd_params in kd_resolutions.items():
                    ...
                    if arcpy.Exists(slice_features_path):
                        ...
                        kd_raster = self._do_kd(clustered_points, kd_raster_name, search_radius, cell_size)

                        if kd_raster is not None:
                            con_raster_name = '{}_{}_{}con'.format(source_id, theme_id, kd_id)
                            con_raster = self._do_con(kd_raster, con_raster_name)
                            ...
                            if con_raster is not None:
                                ...
                                slice_raster = self._do_slice(con_raster, slice_raster_name)

                                if not keep_working:
                                    arcpy.Delete_management(con_raster)

                                if slice_raster is not None:
                                    print('{}: Vectorising Raster: {}'.format(datetime.datetime.now(), slice_features_name))
                                    if slice_raster is None:
                                        print('Slices not defined - Skipping')
                                        theme_result[kd_id]['slices'] = None
                                    else:
                                        arcpy.RasterToPolygon_conversion(in_raster=slice_raster,
    ...
    @staticmethod
    def _do_kd(clustered_points, kd_result_name, search_radius, cell_size):
        ...
        if arcpy.Exists(kd_result_path):
            ...
            cluster_lyr = arcpy.MakeFeatureLayer_management(in_features=clustered_points,
                                                            out_layer='tmp_layer',
                                                            where_clause='CLUSTER_ID > 0')

            row_count = int(arcpy.GetCount_management(cluster_lyr)[0])
            print('{} clustered points found'.format(row_count))
            if row_count < privacy_threshold:
                ...
                result = KernelDensity(in_features=cluster_lyr,
    ...
    @staticmethod
    def _do_con(kd_raster, con_name):
        ...
        if arcpy.Exists(con_raster_path):
            ...
            result = Con(in_conditional_raster=kd_raster,
                         in_true_raster_or_constant=kd_raster,
                         in_false_raster_or_constant="",
                         where_clause="Value > 0")

            if keep_working:
                result.save(con_raster_path)

            return result

    @staticmethod
    def _do_slice(con_raster, slice_name):
...
class SliceProcessor(object):
            ...
            for theme_id, theme_result in source_result.items():
                for kd_id in kd_resolutions.keys():
                    ...
                    if kd_result_path:
                        ...
                        in_slices = slice_params.get('slices', None)
                        if in_slices:
                            ...
                            for tier in range(slices, 0, -1):
                                result = self._process_slice(in_slices=in_slices, source_id=source_id, theme_id=theme_id, kd_id=kd_id, tier=tier)
                                in_slices = result
    ...
    def _process_slice(self, in_slices, source_id, theme_id, kd_id, tier):
        ...
        if dissolved:
            ...
            if tier_features is None:
                arcpy.FeatureClassToFeatureClass_conversion(in_slices, kd_working_gdb, tier_file_name)
            ...
            wkg_features = tier_features + 'predissolve'
            if arcpy.Exists(wkg_features):
                arcpy.Delete_management(wkg_features)
            arcpy.CopyFeatures_management(tier_features, wkg_features)
