...
default_service_url = r'https://services9.arcgis.com/7eQuDPPaB6g9029K/arcgis/rest/services/Daily_COVID_19_Cases_by_Postcode/FeatureServer/0'


class DailyCovid19CasesByPostcode(object):
    ...
    def update(self, rows, allow_deletes=False):
        ...
        :param rows: [{'postcode': str, 'date_code': str, 'cases': int, 'total_cases': int, 'xy': {'x': float, 'y': float}}]
        :type rows: [dict]
        ...
        deletes = []
        updates = []
        update_fields = [self.postcode_field, self.date_field, self.date_code_field, self.cases_field, self.total_cases_field]
        query_result = self.layer.query(out_fields=update_fields)
