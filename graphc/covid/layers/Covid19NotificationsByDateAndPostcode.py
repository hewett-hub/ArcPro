...
default_service_url = r'https://services9.arcgis.com/7eQuDPPaB6g9029K/arcgis/rest/services/COVID19_Notifications_by_Date_and_Postcode/FeatureServer/0'
...
class Covid19NotificationsByDateAndPostcode(object):
    ...
    def synchronize_with_authority(self, authority):
        """

        :param authority:
        :type authority: graphc.covid.admin.AuthorityData.NotificationsByDateAndPostcode
        :return:
        :rtype:
        """
...
class UpdateFromSourceTask(object):
    ...
    def full_update(self):
        ...
        sorted_data = sorted(source_data, key=lambda x: (x[self.nsw_notifications_source.postcode_field], x[self.nsw_notifications_source.date_field]))
        ...
        for row in sorted_data:
            date_str = self.feature_service.format_date_string(date_string=row[self.nsw_notifications_source.date_field], in_format='%Y%m%d')
